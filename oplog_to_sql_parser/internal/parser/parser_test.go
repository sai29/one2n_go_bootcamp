...
func TestParser_GetSqlStatements(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "Insert Oplog",
			oplog:   parser.Oplog{Op: "i", Namespace: "student.students", Record: map[string]interface{}{"_id": "64798c213f273a7ca2cf516a", "age": 25, "name": "Nathan Lindgren", "subject": "Maths"}, UpdateColumns: map[string]interface{}{"_id": "64798c213f273a7ca2cf516a"}, TableCreated: false, TimeStamp: map[string]interface{}{"I": 2, "T": 1.685687329e+09}},
			want:    []string{"CREATE SCHEMA student;", "CREATE TABLE IF NOT EXISTS student.students (_id VARCHAR(255) PRIMARY KEY, age FLOAT, name VARCHAR(255), subject VARCHAR(255));", "INSERT INTO student.students (_id, age, name, subject) VALUES ('64798c213f273a7ca2cf516a', 25, 'Nathan Lindgren', 'Maths');"},
			wantErr: false,
		},
		{
			name:    "Update Oplog",
		...
		{
			name:    "Delete Oplog",
			oplog:   parser.Oplog{Op: "d", Namespace: "student.students", Record: map[string]interface{}{"_id": "64798c213f273a7ca2cf516a"}, UpdateColumns: map[string]interface{}(nil), TableCreated: false, TimeStamp: map[string]interface{}{"I": 6, "T": 1.685687331e+09}},
			want:    []string{"DELETE FROM student.students WHERE _id = '64798c213f273a7ca2cf516a';"},
			wantErr: false,
		},
		{
			name:    "Another Insert",
